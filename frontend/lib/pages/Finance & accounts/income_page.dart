...
class _IncomePageState extends State<IncomePage> {
  ...
  void _applyFiltersClientSide() {
    try {
      ...
      _allFilteredIncomes = _allIncomesCache.where((income) {
        try {
          // Category filter
          if (_selectedCategory != 'All' &&
              income.incomeCategoryName != _selectedCategory) {
            return false;
          }
  ...
  double _getTotalIncomes() {
    return _allFilteredIncomes.fold(0.0, (sum, income) => sum + income.amount);
  }
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _buildSummaryCard(
                        'Total Incomes',
                        '${_allIncomesCache.length}',
                        Icons.receipt,
                        const Color(0xFF4CAF50),
                      ),
                      _buildSummaryCard(
                        'Total Amount',
                        'Rs. ${_getTotalIncomes().toStringAsFixed(2)}',
                        Icons.attach_money,
                        const Color(0xFF2196F3),
                      ),
                      _buildSummaryCard(
                        'This Month',
                        '${_getThisMonthIncomes()}',
                        Icons.calendar_today,
                        const Color(0xFF8BC34A),
                      ),
                      _buildSummaryCard(
                        'Avg. Income',
                        'Rs. ${_getAverageIncome().toStringAsFixed(2)}',
                        Icons.trending_up,
  ...
  int _getThisMonthIncomes() {
    final now = DateTime.now();
    final thisMonth = DateTime(now.year, now.month);
    return _allIncomesCache.where((income) {
      try {
  ...
  double _getAverageIncome() {
    if (_allIncomesCache.isEmpty) return 0.0;
    return _getTotalIncomes() / _allIncomesCache.length;
  }
